...
class TranslateInENGCell: UICollectionViewCell {
    ...
    public func setupNeededWord(indexPath : Int) {
        ...
        answerButtons[y % 4].setTitle(word!.word, for: .normal)
        answerButtons[(y+1) % 4].setTitle(word!.tranEN1, for: .normal)
        answerButtons[(y+2) % 4].setTitle(word!.tranEN2, for: .normal)
        answerButtons[(y+3) % 4].setTitle(word!.tranEN3, for: .normal)
    ...
    @objc func tapTrueAnswer (button : UIButton) {
        ...
        print("Нажали")
        if button.currentTitle == word!.word{
            print("Правильно")
            ...
            for i in 0...3 {
                if answerButtons[i].currentTitle == word?.word{
                    index = i
                }
            }
    ...
    private func setupMainUI() {
        ...
        mainWord.trailingAnchor.constraint(equalTo: viewUnderMainWord.trailingAnchor, constant: -20).isActive = true
